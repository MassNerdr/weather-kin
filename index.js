...
// --- Compact Kindroid scene for Visual Crossing ---

function compassDirection(degrees) {
  if (degrees == null || isNaN(Number(degrees))) return null;
...
function compactCondition(condition) {
  return condition
    .replace("thunderstorming with hail", "storms w/ hail")
    .replace("thunderstorming", "storms")
    .replace("snowing heavily", "heavy snow")
    .replace("snowing lightly", "light snow")
    .replace("snowing", "snow")
    .replace("drizzling", "drizzle")
    .replace("showery", "showers")
    .replace("rainy", "rain")
    .replace("foggy", "fog")
    .replace("overcast", "cloudy");
}

function majorAlert(alerts) {
  ...
  return alerts.find((alert) => {
    ...
    return majorTerms.some((term) => text.includes(term));
  }) ?? null;
}

function appendWithinLimit(parts, next) {
  if (!next) return parts;
  const candidate = [...parts, next];
  return candidate.join(" ").length <= SCENE_CHAR_LIMIT ? candidate : parts;
}

function formatCompactScene(data) {
  const current = data.current;
  const temp = Math.round(current.temperature_2m);
  const feels = current.apparent_temperature == null ? null : Math.round(current.apparent_temperature);
  const code = current.weather_code;
  const condition = compactCondition(WMO_CONDITIONS.get(code) || "unknown");
  ...
  if (data.tomorrow) {
    const high = Math.round(data.tomorrow.temperature_2m_max);
    const low = Math.round(data.tomorrow.temperature_2m_min);
  ...
  if (windSpeed != null) {
    outputParts = appendWithinLimit(outputParts, `Wind ${windDir ? `${windDir} ` : ""}${windSpeed} ${windUnit}.`);
  }
...
async function tick() {
  ...
  try {
    ...
    lastScene = isForecastTick
      ? formatForecast(data)
      : (CONFIG.weatherProvider === "visualcrossing" ? formatCompactScene(data) : formatScene(data));
